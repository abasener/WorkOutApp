...
abstract class MetricChartPoints {
  static List<ChartPoint> forEntries(List<MetricEntry> entries, DateTime cutoff) {
    return entries
  ...
  static List<ChartPoint> workoutDuration(Map<String, double> byDate, DateTime cutoff) {
  ...
  static List<ChartPoint> weeklyBodyweight(List<BodyweightEntry> entries, DateTime cutoff) {
  ...
  static List<ChartPoint> customMetric(List<CustomMetricEntry> entries, DateTime cutoff) {
    return entries
        .where((e) => DateTime.parse(e.date).isAfter(cutoff))
        .map((e) => ChartPoint(DateTime.parse(e.date), e.value))
        .toList();
