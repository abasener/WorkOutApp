...
**There is one codebase, not two.** Every ordinary change — a new feature, a logic fix, a UI tweak — is a plain source edit, and that same source is what both `flutter run` (the `.dev` build) and `flutter build appbundle --release` (the real one) compile from. Testing something on `.dev` and later "shipping it" isn't a porting/merging step; it's just building the *other* target from the same, already-current source. Nothing needs to be copied over, and nothing needs to be stripped back out — the name/icon/header differences below are the *only* things that differ between the two, and each is tied to the build type itself (`debug` vs. `release`), not something manually toggled per build, so a real release can never accidentally carry one of them.
...
**Different launcher icon too, not just the name/package (2026-07-27).** The label/package suffix alone still meant eyeballing which icon on the home screen was which after both got installed a few times in a row — genuinely lost track once (the exact "which one am I looking at" problem this whole dev/release split exists to prevent). Fixed the same way as the label: a debug-only resource override, not a second icon design. `android/app/src/debug/res/{drawable-*,mipmap-*}` now carry a copy of the real launcher icon (both the adaptive foreground and the legacy flat fallback) with the top "head" circle recolored from red to green (`#1D8F2C`, matching `assets/icon/iconDev.png` — the master reference for this recolor) instead of the mark itself changing shape. Same filenames as `android/app/src/main/res` (`ic_launcher_foreground.png`/`ic_launcher.png`), so Gradle's resource merging picks the debug version automatically for `flutter run`/`flutter build apk --debug`, exactly the same mechanism the "TerrapinLift Dev" label override already uses — nothing in `main/` touched, confirmed by extracting the actual icon resource from both a debug APK and the release `.aab` afterward.
- **First attempt (same day) got the placement wrong** — a small green dot added to the icon's bottom-right corner tested fine as a flat PNG preview, but got clipped clean off on the real device: Android's adaptive icon system crops the foreground layer to a circle/squircle mask, and that corner sits outside the visible region once masked, something a flat-image preview doesn't show. Fixed by recoloring the existing, already-centrally-placed head circle instead of adding new content near an edge — nothing added outside the mark's own already-safe area, so there's nothing left for the OS to crop away.
- **The Home screen's AppBar title also gets a `- Dev` suffix in a debug build** (`kDebugMode` from `package:flutter/foundation.dart` — a compile-time constant, `true` only for `flutter run`/`flutter build apk --debug`, always `false` in a real `--release` build, so this can't leak into what ships without a separate mistake). A second, even-lower-effort way to tell the two apart once already inside the app, on top of the icon/label/package split.
