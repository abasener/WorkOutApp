...
**Muscle-highlight diagram is NOT a DB column** — it turned out to be a static, curated `Map<String, List<Muscle>>` in code (`MuscleMap.liftMuscles`, keyed by exercise name) for the original 7 seeded lifts only, since "which muscles does Back Squat hit" is reference data you look up once, not something derived from logged data. The ~68 bulk-added seed exercises (below) deliberately were **not** hand-curated this way — same fallback path as any custom exercise, muscles implied by broad category tags. Curating fine diagrams/cues for dozens of exercises is a lot of manual work for uncertain payoff; revisit per-lift if a specific one turns out to matter. See `00_UX_DESIGN.md`.
...
Current version: **9** (`exercises.equipment_tags` at v4; v5 backfilled the bulk exercise library onto existing installs; `exercises.pinned` at v6, also backfilled; v7 remaps old 5-category `metrics_log.metric_type` soreness keys to the new 14-sub-region keys; v8 adds `exercises.movement_patterns` — Workout Planner phase 1; v9 adds `workout_templates`/`workout_template_days`/`planned_sessions` and seeds the default template — Workout Planner phase 2, `10_WORKOUT_PLANNER.md`).
